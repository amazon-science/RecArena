...
for (
    ...
    for loss_type in loss_types:
        ...
        if model_name == "RecM":
            ...
            graph_sampler = (
                GraphRandomSampler(
                    num_items=dataset.num_items, num_negatives=5, seed=42
                )
                if needs_negatives
                else None
            )
            ...
            # For BPR loss, sample one per user per epoch (efficient)
            # For BCE loss, use all interactions (pointwise)
            sample_per_user = loss_type == "bpr"
            datamodule = RecDataModule(
                dataset,
                format="implicit",
                model_type=model_type,
        ...
        try:
            ...
            if len(all_predictions) == 0:
                print(f"⚠️  {model_name} + {loss_type.upper()} - SKIPPED: No test data available")
                continue
