...
class LLaDA4Rec(BERT4Rec):
    ...
    # Diffusion compute_loss forms full-vocab logits (dense table gradient).
    ...
    def forward_process(self, sequences, sequence_lengths):
        ...
        position_mask = torch.arange(seq_len, device=device).unsqueeze(
            0
    ...
    def compute_loss(self, batch) -> torch.Tensor:
        ...
        masked_sequences, mask_indices, p_mask = self.forward_process(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if "target" in batch and "negatives" not in batch:
            ...
            logits = torch.matmul(
                hidden_states, self.item_embedding.weight.transpose(0, 1)
            )
    ...
    def predict_next(self, sequences, sequence_lengths):
        """Predict the next item with a SINGLE diffusion forward pass.

        For single-next-item prediction there is exactly ONE masked position
        (appended at the end), so the iterative LLaDA generation loop is
        redundant: every step ran an identical forward over the same input and
        only the final logits at the trailing MASK are used. The previous code
        executed ``diffusion_steps`` (default 50) + 1 identical forwards per
        batch -- ~50x wasted compute with no effect on the result (the loop's
        ``torch.where`` unmasking never changes the input consumed by the next
        step's scoring of the trailing position, and confidence was computed but
        never used). We short-circuit to one forward. Multi-token generation
        that genuinely needs iterative refinement lives in ``generate_sequence``.
        ...
        with torch.no_grad():
            # Extend sequence with one masked position for the next item.
            x = torch.cat(
                [
                    sequences,
                    torch.full((batch_size, 1), self.mask_token_id, device=device),
                ],
                dim=1,
            )

            # Single forward; take logits at the trailing MASK position.
        ...
        for step in range(steps):
            ...
            if num_to_unmask > 0:
                _, top_indices = torch.topk(
                    confidence.view(batch_size, -1), k=num_to_unmask, dim=-1
                )
