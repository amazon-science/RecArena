...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(norm, type):
            ...
            self.norm = VALID_NORMALIZATION_LAYERS[norm](d_model)
        else:
            self.norm = norm

    def _lazy_import_mamba(self, mamba_version):
        """Lazily import Mamba or Mamba2 based on the provided version."""
        if ResidualBlock.MambaBlock is None:
            try:
...
class MambaOriginal(nn.Module):
    def __init__(self, config):
        ...
        norm_layer = VALID_NORMALIZATION_LAYERS.get(config.norm, LayerNorm)
